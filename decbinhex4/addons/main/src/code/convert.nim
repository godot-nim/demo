...
proc convert_from_binary*(input_binary: string):string =
  ...
  let result_hex:string = strip(result_decimal.toHex, trailing = false, chars = {'0'})
...
proc convert_from_hexadecimal*(input_hex:string):string =
  ...
  for c in input_hex:
    ...
    lengthvar_hex -= 1
  let result_binary:string = strip(result_decimal.toBin(63), leading = true, trailing = false, chars = {'0'})
